...
timer_sleep (int64_t ticks)
{
  ...
  //add the OS start time plus given ticks for wake up time.
  current->wake_time = start + ticks;
  intr_disable();
  //push the current thread element on to the sleeping_threads list.
  list_push_back(&sleeping_threads, &current->wakeup);
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  //I found Thread_foreach(), copied it for iterating through the sleeping_threads list.
  struct list_elem *e;
  ...
  for (e = list_begin (&sleeping_threads); e != list_end (&sleeping_threads);
       e = list_next (e))
    {
      struct thread *t = list_entry (e, struct thread, wakeup);
      int64_t current_time = timer_ticks();
      if (t->wake_time <= current_time) {
        thread_unblock(t);
        list_remove(&t->wakeup);
      }
    }
}
